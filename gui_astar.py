    ...
    try:
        ...
        if rv == "start":
            if not can_cells.find_withtag("start"):
                can_cells.itemconfig(
                    current_tag, fill=COLOR_START, tags=("cells", "start")
                )
        elif rv == "end":
            ...
            for tags in brush_tags:
                can_cells.itemconfig(
                    tags,
                    fill=COLOR_GRID,
                    tags=("cells", "normal", cell_id_reversed[tags]),
                )
...
def create_cells():
    ...
    try:
        ...
        can_cells = Canvas(
            main_frame,
            width=number_of_cells * size_of_cell - size_border_cell - 1,
            height=number_of_cells * size_of_cell - size_border_cell - 1,
        ...
        for cell in cell_objlist:
            cell_id[cell.name] = can_cells.create_rectangle(
                cell.coord_x,
                cell.coord_y,
                cell.coord_x2,
                cell.coord_y2,
                ...
                tags=("cells", "normal", cell.name),
                width=size_border_cell,
            )
...
def check_entries(a, b, c):
    ...
    try:
        ...
        label_box_cells_var.set(
            "\N{MULTIPLICATION SIGN}  "
            f" {number_of_cells} = {number_of_cells * number_of_cells}"
        )
...
main_frame = Frame(
    master_frame, width=GRID_WIDTH, height=GRID_HEIGHT, style="main_frame.TFrame"
)
...
text_num_cells = Label(
    cell_options_frame, text=f"row \N{MULTIPLICATION SIGN} col [max={MAX_ENTRY}]"
)
label_box_cells = Label(cell_options_frame, textvariable=label_box_cells_var)
entry_num_cells = Entry(
    cell_options_frame,
    textvariable=entry_num_cells_var,
    justify="right",
    width=3,
    validate="key",
)
entry_num_cells.configure(
    validatecommand=(entry_num_cells.register(test_val), "%d", "%P", "%i")
)
...
radio_start = Radiobutton(
    radio_frame, text="START", variable=radio_value, value="start"
)
...
radio_brush_small = Radiobutton(
    brush_frame, text="S", variable=radio_brush, value="small"
)
radio_brush_medium = Radiobutton(
    brush_frame, text="M", variable=radio_brush, value="medium"
)
...
combo_algorithms = Combobox(
    pre_run_frame, textvariable=algorithm_var, values=list_of_algorithms
)
check_drawing = Checkbutton(
    pre_run_frame, text="draw during operation", variable=drawing_var
)
